...
MAINTENANCE_ITEMS = [
    {"name": "Engine Oil & Oil Filter", "status": "N/A", "image": "assets/maintenance_icon/engine_oil.png", "color": "#fdd835"},
    ...
    {"name": "Transmission Fluid", "status": "N/A", "image": "assets/maintenance_icon/transmission_fluid.png", "color": "#fdd835"},
    {"name": "Fuel Filter", "status": "N/A", "image": "assets/maintenance_icon/fuel_filter.png", "color": "#fdd835"},
    {"name": "Coolant (Radiator Fluid)", "status": "N/A", "image": "assets/maintenance_icon/coolant.png", "color": "#fdd835"},
    {"name": "Battery", "status": "N/A", "image": "assets/maintenance_icon/battery.png", "color": "#fdd835"},
    {"name": "Brake Fluid and Pads", "status": "N/A", "image": "assets/maintenance_icon/brake_fluids-and_pads.png", "color": "#fdd835"},
]
...
def fetch_maintenance_data(user_id, vehicle_id, page):
    try:
        ...
        maintenance_ref = db.collection("users").document(user_id).collection("vehicles").document(vehicle_id).collection("maintenance").stream()
        maintenance_data = []
        for doc in maintenance_ref:
            ...
            try:
                ...
                # Ensure last_maintenance_date is valid
                ...
                if last_maintenance_date:
                    ...
                    logging.warning(f"Invalid or missing last_maintenance_date for {item['name']}: {last_maintenance_date_str}")

                # Set default component_type if missing
                if not data.get("component_type"):
                    ...
                    # Save the default component_type to the database
                    db.collection("users").document(user_id).collection("vehicles").document(vehicle_id).collection("maintenance").document(item["name"]).set(
                        data, merge=True
                    )

                # Calculate status and schedule if valid data is available
                if data["last_maintenance_date"] != "N/A" and data["component_type"]:
                    try:
                        # Calculate km_since_last
                        last_maintenance_km = data.get("last_maintenance_km", 0)
                        km_since_last = current_km - last_maintenance_km
                        logging.info(f"Calculated km_since_last for {item['name']}: {km_since_last} (current_km={current_km}, last_maintenance_km={last_maintenance_km})")
                ...
                maintenance_data.append({
                    ...
                    "image": item["image"],  # Use MAINTENANCE_ITEMS image
                    "color": item["color"]
                })
            except Exception as ex:
        ...
        # Ensure all maintenance items are initialized
        for item in MAINTENANCE_ITEMS:
        ...
        # Validate maintenance_data entries before returning
        validated_maintenance_data = []
        for entry in maintenance_data:
            if not isinstance(entry, dict):
                logging.error(f"Invalid maintenance data entry: {entry}")
                continue
            required_keys = ["name", "status", "last_maintenance_date", "estimated_maintenance_schedule", "component_type", "km_remaining", "prediction_reasons", "image", "color"]
...
def dashboard_page(page: ft.Page):
    ...
    # Remove conflicting window size settings (already set in main.py)
    ...
    try:
        ...
        def refresh_vehicles():
            ...
            try:
                ...
                page.controls.append(ft.Text(f"Error fetching vehicles: {str(ex)}", color=ft.colors.RED, size=12, font_family="Montserrat"))
                page.update()
        ...
        def on_vehicles_snapshot(col_snapshot, changes, read_time):
            ...
            try:
                ...
                page.controls.append(ft.Text(f"Error updating vehicles: {str(ex)}", color=ft.colors.RED, size=12, font_family="Montserrat"))
                page.update()
        ...
        try:
            ...
            page.controls.append(ft.Text(f"Error initializing vehicle updates: {str(ex)}", color=ft.colors.RED, size=12, font_family="Montserrat"))
            page.update()
                                    ...
                                    ft.Column(
                                        ...
                                        controls=[
                                            ft.IconButton(
                                                ...
                                                padding=ft.padding.only(top=40)  # Lower the logout button
                                            ),
                                            ft.IconButton(
                    ...
                    ft.Row(
                        controls=[
                            ft.Container(
                                ...
                                margin=ft.margin.only(left=5,top=-20),
                            ),
                            ft.Container(
                                content=ft.ElevatedButton(
                                    content=ft.Row(
                                ...
                                margin=ft.margin.only(right=5,top=-20),
                            ),
                        ],
                        alignment=ft.CrossAxisAlignment.CENTER,
                        spacing=8,
                        ...
                        ft.Row([
                            ft.ElevatedButton(
                                content=ft.Row([
                                    ...
                                    ft.Text("Add Vehicle", color="white", size=16, weight="bold", font_family="Montserrat"),
                ...
                selected_car.current.content = ft.Column([
                    ...
                    ft.Row([
                        ...
                        ft.IconButton(
                            ...
                            icon_color="#3685cd",
                            icon_size=40,
                            visible=len(vehicles) < 3 or vehicle_index < len(vehicles) - 1,
                            on_click=lambda e: navigate_vehicle(1),
        ...
        def navigate_vehicle(direction):
            ...
            delete_confirmation.current.visible = False
            logging.info(f"Navigating vehicle, direction={direction}, vehicle_index={vehicle_index}, vehicles_count={len(vehicles)}")
            update_vehicle_display()
                    ...
                    try:
                        ...
                        def on_item_exit(e, cr=container_ref):
                            if cr.current:
                                ...
                                page.update()
                        # Use the MAINTENANCE_ITEMS image, not COMPONENT_DATA
                        ...
                        logging.error(f"Error rendering maintenance card for vehicle {vehicle_id}, item {item.get('name', 'unknown')}: {str(ex)}")
                        continue
        ...
        def delete_vehicle(doc_id):
            ...
            try:
                ...
                error_message = ft.Text(f"Error deleting vehicle: {str(ex)}", color=ft.colors.RED, size=12, font_family="Montserrat")
            ...
            controls=[
                ft.Stack([
                    ft.Container(
                        ...
                        height=800,  # Match window height
        ...
        return ft.View(
            "/dashboard",
            controls=[
                ft.Text("Unable to load dashboard. Please try again later.", color=ft.colors.RED, font_family="Montserrat"),
                ft.ElevatedButton(
                    "Retry",
